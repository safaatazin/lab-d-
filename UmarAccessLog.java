...
public class AccessLog {
	...
	private int userID;
	private int doorID;
	private static final String REGEX = "^(\\d{4})(0[1-9]|1[0-2])(0[1-9]|[12]\\d|3[01])\\.(\\d{2})([0-5][0-9])([0-5][0-9]) User:(.{4}) Door:(.{3})$";
	...
	public AccessLog(String logLine) throws IllegalArgumentException {
		...
		if (matchFound == true) {
			...
			this.userID = Integer.parseInt(match.group(7));
			this.doorID = Integer.parseInt(match.group(8));
		} else {
	...
	public void setDay(int day) throws IllegalArgumentException {
		...
		if (this.month == 4 || this.month == 6 || this.month == 9 || this.month == 11) {
			...
			boolean isLeapYear = (this.year % 4 == 0 && (this.year % 100 != 0 || this.year % 400 == 0));
			if (isLeapYear && day > 29) {
				throw new IllegalArgumentException("Not a valid day for February in a leap year: " + day);
			} else if (!isLeapYear && day > 28) {
				throw new IllegalArgumentException("Not a valid day for February in a non-leap year: " + day);
			}
		}
		this.day = day;
	}
